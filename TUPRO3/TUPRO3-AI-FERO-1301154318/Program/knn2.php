<?php
	...
	for ($i=0; $i < 1000 ; $i++) { 
		...
		$hasil[$i]['Like'] = $data_test[$i][0];
		$hasil[$i]['Provokasi'] = $data_test[$i][1];
		$hasil[$i]['Komentar'] = $data_test[$i][2];
		$hasil[$i]['Emosi'] = $data_test[$i][3];
		$hasil[$i]['Hoax'] = $hoax[$i];

	}

	$header = array("Like","Provokasi","Komentar","Emosi","Hoax");
